...
// A database in the backend is a system used to store and manage data for an application. It allows the backend server to save, retrieve, update, and delete data such as user information, product details, orders, etc. The database ensures that all the data is stored securely and can be accessed whenever needed by the application.

// For example, when you use a social media app, your profile information, posts, and messages are stored in a backend database.

// Types of Databases Used in the Backend?

// Relational Databases (SQL Databases):Examples: MySQL, PostgreSQL, SQLite, Microsoft SQL Server, Oracle.

// Description: Relational databases use structured tables to store data. These tables have predefined schemas (defined columns and data types), and the data is related through foreign keys. SQL (Structured Query Language) is used to perform queries and operations on the data.
// Use Cases: Suitable for applications requiring complex queries, data integrity, and structured data relationships. Commonly used in finance, enterprise applications, e-commerce, and CRM systems.
// Data is organized into structured tables with fixed schemas. Great for consistent and related data.


// Non-Relational Databases (NoSQL Databases):Examples: MongoDB, DynamoDB, Cassandra, Redis, CouchDB, Neo4j.

// Description: NoSQL databases provide a flexible schema-less structure, meaning data can be stored in various formats, such as JSON documents (document databases), key-value pairs (key-value stores), wide-column stores, or graphs (graph databases). They are designed to handle large volumes of unstructured or semi-structured data.
// Use Cases: Ideal for applications requiring scalability, flexibility, and fast performance, such as social networks, real-time analytics, content management systems, IoT applications, and mobile apps.

// what is the Mongoose?
// it is library that helps to connect to MangoDB database and perform operations on the database.
// Mongoose helps define the structure of your data using schemas.
//  Mongoose lets you create schemas, which define the shape and structure of the data you store in MongoDB.
// It makes common database tasks (like saving, finding, or updating data) easier.

// What is the Schema?
// it is the blueprint of the database.it defines the structure of the database.

// MODEL: It is a constructor function that takes the  schema and creates an instance of the document. it represents the collection in the database.


// MongoDB Compass is a graphical user interface (GUI) tool designedMongoDB Compass is a graphical user interface (GUI) tool designed
...
mongoose
  .connect(
    "mongodb+srv://parvatiawrul117:parvatiawrul117@cluster0.kjsya.mongodb.net/sample_mflix?retryWrites=true&w=majority&appName=Cluster0",
    {
